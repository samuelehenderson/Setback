...
io.on('connection', function(socket) {
  ...
  socket.on('create-room', function(data, cb) {
    ...
    rooms[code] = {
      ...
      trumpSuit: null,
      hands: {},
      kitty: [],         // NEW: middle pile
      remainingDeck: [],  // NEW: leftover cards after deal
      trickPlays: [],
      trickLeader: -1,
      currentPlayer: -1,
      tricksWon: {},
      discardsDone: {}    // NEW: track who has discarded
    };

    socket.join(code);
    cb({ success: true, code: code });
    console.log('Room ' + code + ' created (' + maxPlayers + 'p) by ' + data.name);
  });
  ...
  socket.on('play-card', function(data) {
    ...
    if (room.trickPlays.length === room.maxPlayers) {
      ...
      if (handOver) {
        ...
        setTimeout(function() { broadcastGameState(room); }, 2000);
      }
    } else {
  ...
  while (checked < room.maxPlayers) {
    if (room.bids[next] === undefined) {
      ...
      if (next === room.dealer && everyoneElseBid && room.highBid === 0) {
        ...
        console.log(room.players[next].name + ' forced to bid 3 (dealer stuck)');
